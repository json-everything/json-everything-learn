...
3. Read additional files by concern:
   ...
   - `apis.instructions.md` when editing `LearnJsonEverything/Services/Hosts/` or lesson host behavior.
   - `lesson-writing.instructions.md` when authoring or editing lesson JSON files.
   - `domain-knowledge.instructions.md` when implementing behavior tied to the lesson system or JSON library usage.
   - `instructions.instructions.md` when editing `.instructions.md` files.
4. For C# style and formatting, treat `.editorconfig` and `LearnJsonEverything.sln.DotSettings` in the repo root as authoritative.
...
- Keep changes aligned with the repository's app-first design (Blazor WASM app and lesson content).
